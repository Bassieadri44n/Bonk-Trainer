...
pub fn calculate_trade(global_state: &GlobalState, rt: u128, rs: u128, bs: u128) -> Result<u64> {
    ...
    let res = psn.checked_mul(bs).unwrap().checked_div(divee).unwrap().checked_div(2).unwrap();
    Ok(res as u64)
}

pub fn calculate_bonks_buy(global_state: &GlobalState, my_sol: u64, total_sol: u64) -> Result<u64> {
    calculate_trade(
        global_state,
        my_sol as u128,
        total_sol as u128,
        global_state.market_bonks as u128,
    )
}

pub fn calculate_bonks_sell(
    global_state: &GlobalState,
    my_bonks: u64,
    total_sol_amt: u64,
) -> Result<u64> {
    msg!("my bonks {}", my_bonks);
    msg!("global_state.market_bonks {}", global_state.market_bonks);
...
pub fn get_bonks_since_last_hatch(
    ...
    let mut seconds_passed = cur_timestamp
        ...
        .unwrap();
    if seconds_passed > bonks_per_miner {
        seconds_passed = bonks_per_miner;
    }
