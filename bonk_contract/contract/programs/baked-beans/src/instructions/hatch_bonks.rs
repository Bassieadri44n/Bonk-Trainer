...
pub fn handle(ctx: Context<HatchBonks>) -> Result<()> {
    ...
    msg!(
        "hatch ctx.accounts.user_state.claimed_bonks : {}",
        ctx.accounts.user_state.claimed_bonks
    );
    let bonks_used = ctx.accounts
        .user_state
        .claimed_bonks
        .checked_add(get_bonks_since_last_hatch(
            &ctx.accounts.user_state,
            cur_timestamp,
            ctx.accounts.global_state.bonks_per_miner,
        )?)
        .unwrap();

    msg!("hatch bonks_used: {}", bonks_used);
    msg!(
        "hatch ctx.accounts.global_state.bonks_per_miner: {}",
        ctx.accounts.global_state.bonks_per_miner
    );
    let new_miners = bonks_used
        .checked_div(ctx.accounts.global_state.bonks_per_miner)
        .unwrap();
    ...
    ctx.accounts.user_state.claimed_bonks = 0;
    ...
    if ctx.accounts.user_state.referral_set == 1 {
        ...
        ctx.accounts.referral_state.claimed_bonks = ctx.accounts
            .referral_state
            .claimed_bonks
            .checked_add(bonks_used / 8)
            .unwrap();
    }

    ctx.accounts.global_state.market_bonks = ctx.accounts
        .global_state
        .market_bonks
        .checked_add(bonks_used / 5)
        .unwrap();
