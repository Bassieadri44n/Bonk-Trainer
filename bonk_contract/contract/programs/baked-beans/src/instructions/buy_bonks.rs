...
pub fn handle(ctx: Context<BuyBonks>, sol_amount: u64) -> Result<()> {
    ...
    let mut bonks_bought =
        calculate_bonks_buy(&accts.global_state, sol_amount, accts.vault.lamports())?;

    let bonks_bought_fee = dev_fee(&accts.global_state, bonks_bought)?;
    bonks_bought = bonks_bought.checked_sub(bonks_bought_fee).unwrap();
    ...
    accts.user_state.claimed_bonks = accts
        .user_state
        .claimed_bonks
        .checked_add(bonks_bought)
        .unwrap();
    Ok(())
}
